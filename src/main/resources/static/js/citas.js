...
// HELPER: campo ID de sólo lectura
...
// FILTRO ESPECIALISTAS POR ÁREA
function filtrarEspecialistasCita() {
    const area = v('f_area_cita');
    const cont = document.getElementById('checkboxes-especialistas');
    if (!area) {
        cont.innerHTML = '<p class="text-muted small mb-0">Seleccione un área primero.</p>';
        return;
    }
    const fil = _especialistasCache.filter(e => e.area === area);
    if (!fil.length) {
        cont.innerHTML = '<p class="text-muted small mb-0">No hay especialistas en esta área.</p>';
        return;
    }
...
function soloUnoEspecialista(el) {
    ...
    if (!el.checked) {
        document.getElementById('seccion-detalle').innerHTML = '';
        return;
    }
    ...
    // Filtrar horarios de ese especialista
    ...
    if (horSelect) {
        ...
        horSelect.innerHTML = filtrados.length
            ...
            filtrados.map(h => `<option value="${h.idhorario}">Horario #${h.idhorario} · ${h.duracion} · ${h.disponible ? 'Disponible' : ' Ocupado'}</option>`).join('')
            : `<option value="">Sin horarios para este especialista</option>`;
    }

    // Mostrar campos de detalle según el área
    const seccionDetalle = document.getElementById('seccion-detalle');
    if (seccionDetalle) {
        seccionDetalle.innerHTML = renderDetalleFields(esp.area);
    }
}
...
// CAMPOS DE DETALLE SEGÚN ÁREA
function renderDetalleFields(area, d = {}) {
    if (!area) return '';
    const titulo = {
        'Medicina':   ' Detalle Médico',
        'Psicología': ' Detalle Psicológico',
        'Nutrición':  ' Detalle Nutricional'
    }[area] ?? 'Detalle';
    ...
    return `
        <hr class="my-2">
        <p class="fw-bold text-primary mb-2">${titulo}</p>
        ${campos}`;
}
...
const forms = {
        ...
        <select class="form-select mb-2 border-primary" id="f2">
            ...
            ${usuarios.map(u => `
                <option value="${u.idUsuario}">
                    ${u.nombre} · ${u.tipoUsuario} · ${u.matricula ?? 'Sin matrícula'}
                </option>`).join('')}
        </select>
    ...
    citas: (d, u, e, h) => {
        ...
        const horFiltrados  = areaActual
            ? h.filter(x => x.especialista === espActual.nombre)
            : h;
        ...
        // Detalle pre-cargado si es edición
        const detalleHtml = areaActual ? renderDetalleFields(areaActual, {}) : '';
        ...
        <div id="checkboxes-especialistas" class="border rounded p-2 mb-2 bg-white" style="min-height:42px;">
            ${checkboxesHtml}
        </div>
        ...
        <div id="seccion-detalle">${detalleHtml}</div>`;
    },
...
async function openModal(s, id = null) {
    ...
    if (s === 'citas') {
        const [u, e, h] = await Promise.all([
            req(`${API}/usuarios`),
            req(`${API}/especialistas`),
            req(`${API}/horarios`)
        ]);
        html = forms[s](d, u, e, h);
    } else if (s === 'expedientes') {
        // Cargamos usuarios y citas para mostrar solo quienes tienen cita
        const [usuarios, citas] = await Promise.all([
            req(`${API}/usuarios`),
            req(`${API}/citas`)
        ]);
        // IDs de usuarios que tienen al menos una cita
        const idsConCita = new Set(citas.map(c => c.usuarioIdUsuario));
        // Solo usuarios con cita y sin expediente aún (idExpediente null)
...
async function guardar() {
    try {
        ...
        if (seccion === 'citas') {
            ...
            // Hora: asegurar formato HH:mm:ss que espera Spring LocalTime
            ...
            // Guardar detalle solo en creación (POST), no en edición
            if (!editId) {
                ...
                if (area === 'Medicina') {
                    const det = { motivoConsulta: v('det_f0'), alergias: v('det_f1'), citaIdCita: idCita };
                    await req(`${API}/detalle-medica`, 'POST', det);
                } else if (area === 'Psicología') {
                    const det = { descripcionEmocional: v('det_f0'), motivoConsulta: v('det_f1'), citaIdCita: idCita };
                    await req(`${API}/detalle-psicologica`, 'POST', det);
                } else if (area === 'Nutrición') {
                    const det = { pesoActual: parseFloat(v('det_f0')), estatura: parseFloat(v('det_f1')), objetivo: v('det_f2'), citaIdCita: idCita };
                    await req(`${API}/detalle-nutricion`, 'POST', det);
                }
            }
            ...
            const expBody = { fechaDeCreacion: v('f0'), historialNotas: v('f1') };
            const expCreado = await req(`${API}/expedientes`, 'POST', expBody);

            // Cargar el usuario completo y vincularlo al expediente
            ...
            await req(`${API}/usuarios/${idUsuario}`, 'PUT', usuarioActualizado);
...
async function cargarTabla() {
    try {
        // En citas precargamos usuarios y especialistas para mostrar nombres en la tabla
        if (seccion === 'citas') {
...
function fila(d) {
    ...
    // Resuelve nombre desde cache (tabla de citas)
    ...
    switch (seccion) {
        case 'usuarios':
            return `<tr><td>${d.idUsuario}</td><td>${d.matricula}</td><td>${d.nombre}</td><td>${d.correo}</td><td>${d.telefono}</td><td>${d.tipoUsuario}</td><td>${d.idExpediente ?? '—'}</td><td>${btns}</td></tr>`;
        case 'expedientes':
            return `<tr><td>${d.idExpediente}</td><td>${d.fechaDeCreacion}</td><td>${d.historialNotas}</td><td>${btns}</td></tr>`;
        case 'especialistas':
            return `<tr><td>${d.idEspecialista}</td><td>${d.nombre}</td><td>${d.correo}</td><td>${d.telefono}</td><td>${d.area}</td><td>${btns}</td></tr>`;
        case 'horarios':
            return `<tr><td>${d.idhorario}</td><td>${d.duracion}</td><td>${d.disponible ? ' SI' : 'NO'}</td><td>${d.especialista}</td><td>${btns}</td></tr>`;
        case 'citas':
            return `<tr><td>${d.idCita}</td><td>${d.fecha}</td><td>${d.hora}</td><td>${d.estado}</td><td>${nombreUsuario(d.usuarioIdUsuario)}</td><td>${nombreEspecialista(d.especialistaIdEspecialista)}</td><td>${d.horarioIdhorario}</td><td>${btns}</td></tr>`;
        case 'detalle-medica':
            return `<tr><td>${d.idDetalleMedica}</td><td>${d.motivoConsulta}</td><td>${d.alergias}</td><td>${d.citaIdCita}</td><td>${btns}</td></tr>`;
        case 'detalle-nutricion':
            return `<tr><td>${d.idDetalleNutricion}</td><td>${d.pesoActual}</td><td>${d.estatura}</td><td>${d.objetivo}</td><td>${d.citaIdCita}</td><td>${btns}</td></tr>`;
        case 'detalle-psicologica':
            return `<tr><td>${d.idDetallePsicologica}</td><td>${d.descripcionEmocional}</td><td>${d.motivoConsulta}</td><td>${d.citaIdCita}</td><td>${btns}</td></tr>`;
        default:
            return '';
    }
}
